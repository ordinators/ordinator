...
impl GitManager {
    ...
    pub fn init(&self) -> Result<()> {
        ...
        let repo = Repository::init_opts(&self.repo_path, &init_opts)
            .with_context(|| format!("Failed to initialize Git repository at {}", self.repo_path.display()))?;
    ...
    pub fn push(&self, force: bool) -> Result<()> {
        info!("Pushing changes to remote{}", if force { " (force)" } else { "" });
    ...
    pub fn pull(&self, rebase: bool) -> Result<()> {
        info!("Pulling changes from remote{}", if rebase { " (rebase)" } else { "" });
        ...
        remote.fetch(&["refs/heads/master:refs/remotes/origin/master"], Some(&mut fetch_options), None)
...
mod tests {
    ...
    fn test_remote_addition() {
        ...
        // Add remote
        git_manager.add_remote("origin", "https://github.com/user/dotfiles.git").unwrap();
        
        // Verify remote was added
        let repo = Repository::open(temp_dir.path()).unwrap();
        let remote = repo.find_remote("origin").unwrap();
        assert_eq!(remote.url().unwrap(), "https://github.com/user/dotfiles.git");
    }
    ...
    fn test_push_without_remote() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("No remote 'origin' found"));
    }
